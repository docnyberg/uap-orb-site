...
def process_file(fp: Path, mode: str,
                 ...
                 medallion_max_area: float,
                 medallion_min_fill: float):
    ...
    if not is_logo:
        medallion_hit, medallion_box = detect_golden_medallion(
            img,
            max_area_ratio=medallion_max_area,
            min_fill_ratio=medallion_min_fill,
        )
        if medallion_hit:
            is_logo = True
...
def main():
    ...
    p.add_argument("--medallion-max-area", type=float, default=0.4,
                   help="maximum area ratio for medallion detector (default 0.4)")
    p.add_argument("--medallion-min-fill", type=float, default=0.25,
                   help="minimum fill ratio for medallion detector (default 0.25)")
            ...
            for name in files:
                if name.lower().endswith((".jpg",".jpeg",".png",".bmp",".gif")):
                    ...
                    if args.mode == "move":
                        ...
                        if not hit:
                            ...
                            if badge_hit:
                                hit = True
                        if not hit:
                            medallion_hit, _ = detect_golden_medallion(
                                img,
                                max_area_ratio=args.medallion_max_area,
                                min_fill_ratio=args.medallion_min_fill,
                            )
                            if medallion_hit:
                                hit = True
                    ...
                    changed, note = process_file(fp, args.mode, args.min_red, args.min_cyan,
                                                 ...
                                                 args.medallion_max_area,
                                                 args.medallion_min_fill)
                    if changed:
